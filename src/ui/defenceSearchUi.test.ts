...
describe("buildDefenceSearchInput", () => {
  ...
  it("requires at least one attack instead of treating an ally ability as an attack", () => {
    const [scenario] = createDefaultScenarioForms();
    scenario.attacks = [{ ...scenario.attacks[0], id: "ally", moveInput: "", attackerAbilityInput: "バッテリー" }];
    expect(() => buildDefenceSearchInput(createDefaultTargetForm(), [scenario])).toThrow("有効な攻撃条件がありません");
  });

  it("does not infer simultaneous allies from other attack cards", () => {
    const [scenario] = createDefaultScenarioForms();
    scenario.attacks[0].gameType = "doubles";
    scenario.attacks.push({ ...scenario.attacks[0], id: "ally", attackerAbilityInput: "バッテリー" });
    const input = buildDefenceSearchInput(createDefaultTargetForm(), [scenario]);
    expect(input.scenarios[0].hits).toHaveLength(2);
    expect(input.scenarios[0].hits.every((hit) => !hit.allyAbilities?.length && !hit.defenderAllyAbilities?.length)).toBe(true);
  });
