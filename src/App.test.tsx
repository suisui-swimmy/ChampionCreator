...
describe("App", () => {
  ...
  it("keeps box dialogs above mobile sheets and candidate budget values proportional", () => {
    ...
    expect(css).not.toContain(".mobile-search-counts");
    expect(css).not.toContain(".mobile-progress-line");
    expect(css).toMatch(/\.search-progress\s*\{[^}]*min-height:\s*var\(--desktop-control-compact\);/s);
  ...
  it("publishes a static, indexable guide with a responsive real-calculation tutorial", () => {
    ...
    expect(getTutorialMessage("idle", false)).toBe("条件は入力済みです。「配分を探索」を押してください。");
    ...
    expect(tutorialHtml).toContain("条件は入力済みです。「配分を探索」を押してください。");
    expect(tutorialHtml).not.toContain("作業台の「配分を探索」");
  ...
  it("renders the M0 workbench sections", () => {
    ...
    expect(html.match(/class="search-progress" role="progressbar" aria-label="探索進捗" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"/g)).toHaveLength(2);
    expect(html.match(/class="search-progress-fill" style="width:0%" aria-hidden="true"/g)).toHaveLength(2);
    expect(html.match(/<strong>0%<\/strong><span>評価 0 \/ -<\/span>/g)).toHaveLength(2);
    expect(html).not.toContain('class="mobile-search-counts"');
    expect(html).not.toContain("合格 0");
    expect(html).not.toContain("0% / 評価");
    expect(html).not.toContain(">キャンセル<");
    expect(html).toContain(">現在の配分を確認<");
    expect(html).toContain(">配分を探索<");
    expect(html).toContain("候補一覧");
    expect(html).toContain("計算結果");
    expect(html).not.toContain("配分を探索で Worker 経由の候補がここに出ます");
