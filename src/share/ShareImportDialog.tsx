...
import { ShareImportSummary } from "./ShareImportSummary";
...
export function ShareImportDialog({ state, error, hasWork, canImport, scopeLabel, onUse, onSave, onClose }: {
  ...
  return <ShareDialogFrame title="共有された調整を取り込む" onClose={onClose}>
    {state.status === "loading" ? <p role="status">共有内容を確認しています…</p> : state.status === "error" ? <p role="alert">{state.message}</p> : <>
      <ShareImportSummary target={state.shared.document.target} scenarioCount={state.shared.document.scenarios.length} />
    ...
    <div className="share-actions share-import-actions">
      {state.status === "ready" ? <><Button variant="primary" disabled={!canImport} onClick={onUse}>作業に読み込む</Button><Button disabled={!canImport} onClick={onSave}>ボックスに追加</Button></> : null}
      <Button className="share-import-cancel" onClick={onClose}>キャンセル</Button>
    </div>
  </ShareDialogFrame>;
}
