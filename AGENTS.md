...
`guide/` はアプリ本体とは別の利用導線であり、一般ユーザー向けの使い方ガイドとする。操作方法や画面上の挙動を、専門知識を前提としない自然な日本語で分かりやすく簡潔に説明し、表示仕様とバージョンは本体と整合させる。
...
- 簡潔さも重視する。すべての仕様を長々と書くのではなく、ユーザーが迷わない範囲で必要な情報だけを載せる
