...
この文書は、今後の変更でも守る設計境界と作業ルールを定義する。現行機能の技術仕様は `README.md`、変更経緯・実施結果は進捗記録へ分け、完了した開発手順を再開するロードマップとして扱わない。
...
5. 仕様に関係する場合は `README.md`。操作方法や結果の読み方に影響する場合は `guide/` の該当箇所も確認する
...
- シンプルさと密度を両立させるため、冗長な機能/操作説明を画面上に置かず、説明がなくても導線や操作方法が理解できるUIを維持する
...
- 近い言い換えでテストや確認を済ませない
- named bug は共通の判定・表示処理から同じ意味カテゴリへの影響範囲を確認し、指摘された1件だけの例外処理で済ませない
- エラーは、入力不足、resolver 未解決、計算未対応、保存形式不正、Worker error を区別する
- UI で表示する計算説明は、calc 由来の値とアプリ側 HP イベントを区別する

### 公開ガイド

公開URL `/guide`（ソース `guide/`）は、一般ユーザーに「どこで何を入力し、どう計算・保存し、結果をどう読むか」を示す操作ガイドとする。専門知識を前提にせず、簡潔で分かりやすい自然な日本語で、操作に必要な内容だけを記述する。

- 個別のポケモン・フォーム・技・特性・持ち物の対応状況や仕様、対応件数、未対応一覧、計算の詳細・例外はガイドに記載せず、`README.md` にまとめる。内部構造、データ形式、開発・検証の入口も `README.md` で説明し、独立した詳細手順・監査根拠は既存のGit管理文書を参照する
- 操作方法や結果の読み方が変わる場合だけ、ガイドの該当説明を更新する。`README.md` の仕様説明を転載・網羅的に同期せず、詳しい仕様が必要な読者には `README.md` の該当節への短い案内を置く
- 操作例で個別のポケモン名や技名を使う場合は、入力・操作を示すのに必要な範囲に限り、固有仕様や対応状況の解説へ広げない
- 追記前に「読者の次の操作や結果の読み取りに必要か」を確認する。画面を見れば分かる配置の逐一説明、重複した注意書き、変更履歴を増やさず、不要な説明は削る
- 説明する操作・画面表示とバージョンは現行アプリと整合させ、未実装の機能を先行記載しない
- 変更箇所に関係する exact text、画像寸法、公開 asset path、アンカー、PC / モバイル表示を確認する
...
**変更内容を追記するのではなく、該当箇所を変更後の現行仕様として書き直す。変更経緯と実施結果は別の記録へ残す。**

- `README.md` は現行アプリの技術仕様書であり、変更履歴や作業報告の追記先ではない。仕様変更時は既存の該当節を更新し、収まる節がない場合だけ適切な見出しを追加する。末尾や直近の編集箇所へ機械的に追記しない
- 関係する既存記述を検索し、旧仕様の残存、重複、矛盾、参照リンクを確認する。詳細の説明元を決め、他の節は要約と参照に留める。見出し変更時は既存アンカーの維持または参照側の最小修正を行う。無関係な文書全体の改稿や履歴全件の読み込みは毎回要求しない
- 入力、処理、出力、制約、失敗時動作を必要な範囲で説明する。UIは、入力・内部状態・計算・保存への影響、結果の意味・読み取り、操作可否やアクセシビリティ上の保証を技術仕様として残す。操作手順は「公開ガイド」の役割を優先し、操作や結果の読み方が変わる場合だけ更新する
- 配置・余白・装飾だけの変更では、原則READMEを更新しない。左右・上下の位置、列幅、間隔、枠線、画像寸法、CSS・DOM構造を逐一文章化しない。色や表示形式が結果・状態の解釈に必要な場合は、その意味を簡潔に記す。具体的な描画の維持は実装と対応テストで扱い、変更内容・検証結果は進捗記録へ残す。細部を別の節・ガイド・新規文書へ移すだけの更新もしない
- 変更経緯・実施結果・残件は `PROGRESS.md` / `PROGRESS.archive/`、恒久的な監査根拠・独立した詳細手順は既存のGit管理文書へ置く。READMEの理解や監査にローカル専用ログを必須とせず、公開読者が辿れる参照先を保つ。追記先を変えるだけの雑記や薄い文書を増やさない
- 仕様説明に「今回」「添付」「先ほど」「No.Nの指定」など外部の会話・作業履歴に依存する参照を残さない。具体的な節・資料・出典記録へ置き換える一方、互換性、固定依存、出典、確認範囲を特定する日付・版・SHAは保持する
- `AGENTS.md` を更新するのは恒久的な設計境界や作業・検証ルールが変わる場合とする。通常の機能追加だけを理由に詳細仕様や完了履歴を重複転記しない
- 実装・テスト・文書の不一致は対象経路を確認し、推測や制約の緩和で解消しない。未実装機能、未検証のゲーム仕様、公開未確認の状態を実装済み・確認済みと書かず、確定できない点は該当箇所に限界を示す
...
4. 指摘の再現条件と、同じ原因が波及する範囲の代表的な正常系・境界値を回帰テストへ含める
...
変更に関係する狭いテストから実行し、変更種別ごとの必須検証が合格したら終了する。合格後の追加・再実行は、新しい変更、失敗、未解決の懸念がある場合に限る。計算・保存互換性・Worker・UI実表示の必須確認は省略しない。

影響範囲は共通処理とデータ経路から特定し、回帰テストはその範囲の代表例・正常系・境界値を選ぶ。全ポケモン・全技などの追加の総当たり調査は、網羅性が必要な変更や共通原因を絞れない場合に限る。変更種別で必要な既存の全件validatorは維持する。

`npm run build` は型確認を、`npm run check` はvalidator・全体テスト・Security Rulesテスト・buildを含む。最終確認でこれらを使う場合は、含まれる検証を個別に重ねない。早期の原因切り分けに必要な対象テストや `npm run typecheck` は先に実行してよい。

使い分け:

- logic / calc / search / Worker: 対象テスト、`npm test`、`npm run build`
- generated data: 対象 generator、対応 validator、対象テスト、`npm run build`
- UI: 対象テスト、`npm run build`、Browser 確認。`guide/`・privacyなど画面として配信する箇所にも適用する
- release 前、または計算・保存・同期など複数の責務にまたがる実装変更: `npm run check`。変更ファイル数だけで判断しない
- README / AGENTS / 開発用docs / コメントだけ: 内容確認と `git diff --check`。アプリ test / build と version 更新は原則不要
...
- 公開仕様、対応機能、制限の変更は「文書の役割と更新」に従って `README.md` の該当節へ反映し、ガイドは「公開ガイド」の方針に従う
- 未対応を推測実装で埋めず、UI と `README.md` で明示する

## 作業スタイル

- 明確な実装依頼では、依頼範囲内の通常の可逆的な修正から必要な検証まで、追加確認で止まらず進める。計画・診断・監査だけの依頼では、その範囲を守る
- 確認前に既存コードと会話を調べる。上位指示の範囲内でユーザーの明示依頼・訂正を本書やスキルの一般指針より優先し、すでに得た承認を再要求しない。スキルが停止理由になる場合は、該当ファイルへのリンク、指示の原文、今回の作業へ適用する理由を示す
- 子エージェントは「並列で」「子エージェントを使って」などの明示依頼がある場合だけ使う。既定のLuna / maxを維持し、モデル・推論強度の明示指定があればそれに従う。同時実行上限は起動すべき人数ではなく、依頼に必要な独立した作業だけを分担する
- 報告は既存の日本語・口調を引き継ぎ、結果、必要な根拠と検証、残件を簡潔に伝える
- 既存変更を勝手に戻さない
