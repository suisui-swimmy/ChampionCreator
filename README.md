...
- ヘッダーのシングル／ダブル切替で、入力サジェストの基準を選べる（初期値はシングル）
...
技・特性・持ち物の入力候補は、Pokemon Champions Battle Data が提供する `Current` の使用率データをもとに並び替えます。ヘッダーの「シングル」「ダブル」で基準を切り替えられ、選択した基準だけが同じブラウザに保存されます。
