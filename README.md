...
- 調整対象と攻撃カードの5×5性格表へ、同データの上位10件にある実使用率を濃淡・詳細・ARIAで表示する
...
性格は、調整対象と各攻撃カードの5×5能力補正表へ実使用率の濃淡を表示します。セルへマウスを重ねるかフォーカスすると、表の下で性格、能力補正、選択中のシングル／ダブル使用率、順位を確認できます。提供データは上位10件のため、掲載されていない性格は `0%` ではなく `上位外／データなし` と表示します。使い方ガイドのチュートリアル画面では、従来の候補順と使用率表示なしの性格表を維持します。
