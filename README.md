...
- `もうどく` は開始段階を1〜15、`まきびし` は層数を1〜3で指定できます。`オボンのみ` は連続技の各ヒット後にも判定し、同じHP推移内では1回だけ発動します
...
5. 定数ダメージや回復を含める場合は攻撃カードの `HP推移` を開き、HP変化を追加する。対象・発動タイミング・頻度は効果ごとに自動決定され、現在HP依存の直接ダメージ・威力は読み取り専用の `自動` 情報として表示される
6. `計算開始` を押す
7. 候補一覧から配分、残りSP、最厳条件、直接ダメージとHP変化の内訳を確認する
