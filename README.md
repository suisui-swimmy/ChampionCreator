...
- 現在対応するHPイベントは `いのちのたま反動`、`すなあらし`、`どく`、`もうどく`、`やけど`、`ステルスロック`、`まきびし`、Champions版 `しおづけ`、`オボンのみ`、`たべのこし` です
- `ステルスロック` / `まきびし` → 直接攻撃 → `オボンのみ` → `いのちのたま` → ターン終了時の天候 → 状態異常 → `しおづけ` → `たべのこし` の順でHPを更新し、途中でひんしになった場合は後続処理を止めます
- `もうどく` は開始段階を1〜15、`まきびし` は層数を1〜3で指定できます。`オボンのみ` は連続技の各ヒット後にも判定し、同じHP推移内では1回だけ発動します
- HPイベントの対象は効果から自動決定し、`いのちのたま反動` は技使用者、それ以外の対応効果は技の被弾側へ適用します
...
5. 定数ダメージや回復を含める場合は攻撃カードの `HP推移` を開き、HP変化を追加する。対象・発動タイミング・頻度は効果ごとに自動決定される
6. `計算開始` を押す
