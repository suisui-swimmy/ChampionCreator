...
- 直接攻撃に加えて、明示した `いのちのたま反動` と `すなあらしダメージ` を順番どおりに評価する
...
- 現在対応するHPイベントは `いのちのたま反動`（技使用後・技ごと）と `すなあらしダメージ`（ターン終了時・ターンごと）です
- HPイベントの対象は効果から自動決定し、`いのちのたま反動` は技使用者、`すなあらしダメージ` は被弾側へ適用します
...
5. 定数ダメージを含める場合は攻撃カードの `HP推移` を開き、プリセットを追加する。対象・発動タイミング・頻度は効果ごとに自動決定される
6. `計算開始` を押す
...
- HPイベントは選択した効果が発動する前提で評価します。技失敗、`ちからずく`、特殊な天候無効化などの完全な自動発動判定は未対応です
...
- 定数ダメージは現在 `いのちのたま反動` と `すなあらしダメージ` のみ対応し、毒、設置物、回復、接触反動などは未対応です
