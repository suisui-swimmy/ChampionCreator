...
- 道具倍率・特性倍率は通常`自動`で、手動倍率を選ぶと金色の`手動`表示へ切り替わります。選択中の持ち物・特性に自動S補正がある場合は、`自動 → 手動`の置換内容も表示します。手動倍率は自動補正をそのカテゴリだけ置き換える最終Sへの数値上書きで、特性の発動条件や丸め順まで完全に再現するものではありません。`かるわざ`・`スロースタート`の発動状態や、`こだいかっせい`・`クォークチャージ`でSが上昇する状態を直接指定する専用UIも未対応です
