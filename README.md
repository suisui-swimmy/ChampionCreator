...
4. 技の横の `威力` 欄で計算値を確認する。`▲ / ▼` がある技は戦闘中条件に合う威力へ切り替え、欄を押すと条件名や任意入力を確認できる。HP依存技は閉じたロック中に現在HPから自動計算され、ロックを外すと任意値を入力できる
...
- `ふくろだたき` は参加ポケモンごとに1ヒットの威力が異なり、単一の威力欄では正確に扱えないため、現在の直接ダメージ計算には未対応です
