...
使用率データに掲載されていない候補は五十音順で表示します。順位や使用率は表示せず、候補の並び順だけに反映します。使い方ガイドのチュートリアル画面では、従来の候補順を維持します。
...
`generate:champions-usage` はローカル確認用の使用率JSONを `public/data/champions-usage-current.json` へ生成します。このJSONはgit管理せず、公開時もGitHub Actions内で生成します。
