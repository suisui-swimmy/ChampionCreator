...
- 道具倍率・特性倍率の手動指定は、選択中の持ち物・特性による自動補正をそのカテゴリだけ置き換える、最終Sへの数値上書きです。特性の発動条件や丸め順まで完全に再現するものではありません。`かるわざ`・`スロースタート`の発動状態や、`こだいかっせい`・`クォークチャージ`でSが上昇する状態を直接指定する専用UIも未対応です
