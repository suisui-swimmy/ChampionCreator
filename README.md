...
- `ステルスロック` / `まきびし` → HPコスト → 直接攻撃 → ヒットごとの接触反動・`オボンのみ` → 技固有の反動・`いのちのたま` → ターン終了時の天候 → 状態異常 → `しおづけ` → `たべのこし` の順でHPを更新し、途中でひんしになった場合は後続処理を止めます
...
- 技固有のHPコスト、現在HP依存ダメージ・威力、与ダメージ依存反動、特殊反動、`いのちがけ`の使用者ひんしは、選択した技から自動判定します
...
5. 定数ダメージや回復を含める場合は攻撃カードの `HP推移` を開き、HP変化を追加する。対象・発動タイミング・頻度は効果ごとに自動決定され、技固有のHP変化は読み取り専用の `自動` 情報として表示される
6. `計算開始` を押す
...
- 調整対象のHPはシナリオ内で継続し、同じ攻撃カードを複数回使う間は技使用者のHPも継続します。別の攻撃カードは別の仮想敵として扱うため、`はらだいこ`などのHPコストを別カードの攻撃へ引き継ぐ「同じ技使用者」指定は未対応です
