...
5. 定数ダメージや回復を含める場合は攻撃カードの `定数ダメージ・回復` を開き、効果を追加する。対象・発動タイミング・頻度は効果ごとに自動決定される。HP依存技は設定一覧とは別の補足を表示し、各攻撃・各ヒット時点の現在HPから自動計算する
6. `計算開始` を押す
7. 候補一覧から配分、残りSP、最厳条件、直接ダメージと定数ダメージ・回復の内訳を確認する
8. 使いたい候補の `適用` で調整対象へ反映する
